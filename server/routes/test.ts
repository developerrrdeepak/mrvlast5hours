...
export const testEmail: RequestHandler = async (req, res) => {
  try {
    ...
    if (type === 'otp') {
      ...
      res.json({
        success: result,
        message: result ? "Test OTP email sent successfully" : "Failed to send test OTP email",
      ...
      // Test Welcome email
      const result = await emailService.sendWelcomeEmail(email, 'Test Farmer', 15000);
      
      res.json({
        success: result,
        message: result ? "Test welcome email sent successfully" : "Failed to send test welcome email",
        email_service: emailStatus,
        test_data: {
          email,
